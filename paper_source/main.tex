...
\begin{figure}[t]
  ...
  \caption{\textbf{\method{} links semantic claims to supporting views.} The policy uses the same evidence state to choose the next view and the final \textsc{Yes}/\textsc{No}/\textsc{Wait} decision.}
  \label{fig:teaser}
\end{figure}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.96\textwidth]{figures/fig_2v1.png}
  \caption{
  \textbf{Spatial coverage bounds evidence-sufficient decisions.}
  (a) Candidate viewpoints are restricted to valid navigable space.
  Views with direct target visibility provide positive support for a claim,
  whereas partial or context-only observations remain unresolved.
  (b) Different coverage states imply different claim-level decisions:
  sufficient target evidence supports a positive commitment, while partial
  or context-only evidence should trigger deferment rather than an
  unsupported absence decision. This distinction separates
  \emph{missing evidence} from \emph{evidence of absence}.
  }
  \label{fig:coverage}
\end{figure*}
...
\input{tables/table_active_locked_all_baselines.tex}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_locked_trajectory_focus.pdf}
  \caption{\textbf{One held-out ProcTHOR episode.} From the same start and budget, SafeVantage observes the paper towel, answers \textsc{Yes}, and travels 5.4~m versus 13.7~m.}
...
\begin{figure}[t]
  ...
  \includegraphics[
    ...
    keepaspectratio
  ]{figures/fig_grouped_vantage_ablation_cropped.pdf}
...
\begin{table}[H]
  ...
  \label{tab:main}
  \resizebox{\linewidth}{!}{\input{tables/table1_main_operating_points_column.tex}}
\end{table}
...
\begin{table}[H]
  ...
  \begin{adjustbox}{max width=\columnwidth,max height=0.35\textheight}
    \input{tables/table4_causal_evidence.tex}
  \end{adjustbox}
\end{table}
...
%   \begin{adjustbox}{max width=\columnwidth,max height=0.35\textheight}
%   \scriptsize
%   \resizebox{\linewidth}{!}{\input{tables/table_component_ablation.tex}}
%   \end{adjustbox}
% \end{table}
...
\begin{table}[H]
  ...
  \begin{adjustbox}{max width=\linewidth,max height=0.5\textheight}
    \input{tables/table_component_validation.tex}
  \end{adjustbox}
\end{table}
